...
const DetailsTrip = () => {
    ...
    const handleDateSelected = async ({dateString}) => {
        ...
        const parsed = format(parseISO(dateString), 'dd-MM-yyyy')
        const response = await api.get(`/trip/${idTrip}/${parsed}`)
        const data = response.data
                ...
                {!loading ? (
                    ...
                    <BoxDescription>
                        ...
                        <View style={{ 
                            ...
                            {trip.data.days_disponibles?.length > 0 && trip.data.days_disponibles.map((day_disponible) => {
                                return (
                                    <Day>
                                        <Icon
