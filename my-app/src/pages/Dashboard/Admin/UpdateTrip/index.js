...
const UpdateTrip = () => {
    ...
    useEffect(()=>{
        ...
        const subscribe = navigation.addListener('focus', async ()=>{
            ...
            setFormData({ ...data, schedule_initial, schedule_end, duration })
    ...
    const handleSubmit = async () => {
        ...
        try{
            ...
            const img = !image.uri ? image : {name: 'image.jpg', type:"image/jpg", uri: image.uri}
    
            const form = new FormData()
            form.append('city', city.name)
            ...
            if(response.status === 200){
                setLoading(false)
                Alert.alert('Sucess', 'Passeio atualizado com sucesso')
                return navigation.navigate('Passeios')
            }
        }
        catch(errors){
            ...
            setErrors(modifiedForUniqueObject)
            ...
            <BoxForm>
                ...
                <Local>
                    <Picker 
                        ...
                        onValueChange={(city)=> setFormData({...formData, city: {name: city}})}
                    >
                        <Picker.Item 
                            label="Cidade" 
                            value=""/>
